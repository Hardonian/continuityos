...
class AgenticIntelligenceEngine:
    ...
    def __init__(self, llm_endpoint: str = "http://127.0.0.1:11434/v1/chat/completions", model: str = "llama3") -> None:
    ...
    def _build_prompt(self, packet: DecisionPacket) -> str:
        prompt = (
            "You are the Aegis Continuity Intelligence Engine, an AI advisory system for NATO-aligned "
            "Ministries of Defense and strategic logistics planners. "
            "Analyze the following deterministic mathematical mitigation plan generated by the ContinuityOS exact-solver. "
            "Provide a highly professional, defense-grade executive summary, strategic implications, and a list of advisory actions.\n\n"
        )
        
        prompt += f"Corridor Assessed: {packet.assessment.corridor_id}\n"
        prompt += f"Required Mitigations: {len(packet.plan.actions)}\n"
        
        prompt += "\nCompiler Actions Required:\n"
        for act in packet.plan.actions:
            prompt += f"- {act.action_id} (Cost: {act.cost}, Provider: {act.provider_id})\n"
            
        prompt += "\nRespond strictly in valid JSON format with three keys: "
        prompt += "'executive_summary' (string), 'strategic_implications' (string), and 'advisory_actions' (list of strings)."
        return prompt

    async def generate_briefing(self, packet: DecisionPacket) -> SovereignBriefing:
        """Query the local air-gapped LLM to translate a DecisionPacket into a Sovereign Briefing."""
        prompt = self._build_prompt(packet)
        
        payload = {
            "model": self.model,
            "messages": [
                {"role": "system", "content": "You are a military intelligence assistant. Always output valid JSON."},
        ...
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                ...
                return SovereignBriefing(
                    executive_summary=parsed.get("executive_summary", "Briefing generation failed."),
